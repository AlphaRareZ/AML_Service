...
public class LigandMessageConsumer : BackgroundService
{
    private readonly ILogger<LigandMessageConsumer> _logger;
    ...
    public LigandMessageConsumer(IConfiguration configuration, ILogger<LigandMessageConsumer> logger,
        IServiceScopeFactory scopeFactory)
    {
        _logger = logger;
        _scopeFactory = scopeFactory;
        // Match the Python config keys
        _responseQueue = configuration["RabbitMQ:ligand_response_queue"] ?? "response_queue";
        _exchangeName = configuration["RabbitMQ:exchange_name"] ?? "aml_exchange";
        _routingKey = configuration["RabbitMQ:ligand_response_routing_key"] ?? "response_routing_key";
        ...
        try
        {
            ...
            consumer.ReceivedAsync += async (model, ea) =>
            {
                ...
                try
                {
                    ...
                    var result = JsonSerializer.Deserialize<LigandResponseMessage>(messageStr);

                    if (result is not null && result.Success)
                    {
                        var scope = _scopeFactory.CreateScope();
                        var service = scope.ServiceProvider
                            .GetRequiredService<IResponseMessageService<LigandResponseMessage>>();
                        var repo = scope.ServiceProvider.GetRequiredService<ILigandRepository>();
                ...
                {
                    _logger.LogError(ex, "Error processing message [CorrelationId: {Id}]. Reason: {ErrorMessage}",
